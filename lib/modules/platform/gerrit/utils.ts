...
export function mapGerritChangeToPr(
  ...
  const state = mapGerritChangeStateToPrState(change.status);
  return {
    ...
    closedAt: state === 'closed' ? findClosedAt(change) : undefined,
...
function findClosedAt(change: GerritChange): string | undefined {
  const abandonMessage = change.messages
    ?.filter((msg) => msg.tag === 'autogenerated:gerrit:abandon')
    .at(-1);
  if (abandonMessage) {
    return convertGerritDateToISO(abandonMessage.date);
  }
  return undefined;
}
